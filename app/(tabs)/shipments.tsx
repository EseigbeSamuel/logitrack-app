...
export default function ShipmentsScreen() {
  ...
  const renderShipmentCard = (item: Shipment) => {
    return (
      ...
      >
        <View className="flex-row justify-between items-center">
          <View>
            <Text className="text-[15px] font-black font-mono tabular-nums text-foreground">{item.id}</Text>
    ...
    item.status === 'pending' ? 'bg-[#71717A1F]' :
    item.status === 'picked_up' ? 'bg-[#3B82F61F]' :
    item.status === 'in_transit' ? 'bg-[#F59E0B1F]' :
    'bg-[#10B9811F]'
  }`}
          >
            <Text
    ...
    item.status === 'picked_up' ? 'text-[#60A5FA]' :
    item.status === 'in_transit' ? 'text-warning' :
    'text-success'
  }`}
            >
        ...
        {item.delayReason && (
          <View className="flex-row items-center gap-1.5 border-[0.5px] py-1.5 px-2 rounded bg-[#EF44441F] border-danger">
            ...
            <Text className="text-[9px] font-extrabold tracking-wider text-danger">DELAY ALERT: {item.delayReason.toUpperCase()}</Text>
          </View>
        )}
        ...
        <View className="p-3 rounded-md border-[0.5px] bg-background border-border">
          <View className="flex-row items-center gap-2">
            ...
            <Text className="text-[11px] font-semibold flex-1 text-foreground" numberOfLines={1}>
              {item.senderAddress}
            </Text>
          </View>
          <View className="w-[1px] h-2.5 ml-[2.5px] my-0.5 bg-border" />
          <View className="flex-row items-center gap-2">
  ...
  return (
    <View className="flex-1 bg-background">
      <View className="px-4 pt-16 pb-3 border-b ios:pt-16 border-b-card">
        <Text className="text-[22px] font-black text-[#FAFAFA] tracking-[1.5px] text-foreground">
          {activeRole === 'customer' ? 'ROUTE REGISTRY' : 'DISPATCH CONTROL'}
        </Text>
        <Text className="text-[11px] text-[#71717A] font-semibold mt-1 leading-4 text-muted">
          {activeRole === 'customer'
      ...
      {activeRole === 'rider' && (
        <View className="">
          <Pressable
            className={`flex-1 py-2.5 items-center justify-center rounded-md ${riderSubTab === 'my_tasks' ? 'bg-[#27272A] border border-[#3F3F46]' : ''}`}
            onPress={() => setRiderSubTab('my_tasks')}
          >
            <Text
              className={`text-[11px] font-black tracking-wider ${riderSubTab === 'my_tasks' ? 'text-[#CCFF00]' : 'text-muted'}`}
            >
          ...
          <Pressable
            className={`flex-1 py-2.5 items-center justify-center rounded-md ${riderSubTab === 'available_tasks' ? 'bg-[#27272A] border border-[#3F3F46]' : ''}`}
            onPress={() => setRiderSubTab('available_tasks')}
          >
            <Text
              className={`text-[11px] font-black tracking-wider ${riderSubTab === 'available_tasks' ? 'text-[#CCFF00]' : 'text-muted'}`}
            >
      ...
      >
        {activeRole === 'customer' ? (
          <View className="gap-3">
            {/* Active Consignments */}
            <Text className="text-[10px] font-black tracking-[1.5px] mb-1 text-muted">ACTIVE SHIPMENTS</Text>
            {customerActive.length > 0 ? (
              ...
              <View className="border rounded-lg p-6 items-center justify-center gap-2.5 bg-card border-border">
                <Text className="text-xs font-semibold text-center text-muted">No active logistics runs registered.</Text>
              </View>
            )}
            ...
            {customerHistory.length > 0 ? (
              ...
              <View className="border rounded-lg p-6 items-center justify-center gap-2.5 bg-card border-border">
                <Text className="text-xs font-semibold text-center text-muted">No delivery history available yet.</Text>
              </View>
            )}
          </View>
        ) : (
                ...
                {riderTasks.length > 0 ? (
                  ...
                  <View className="border rounded-lg p-6 items-center justify-center gap-2.5 bg-card border-border">
                    ...
                    <Pressable
                      ...
                      onPress={() => setRiderSubTab('available_tasks')}
                    >
                      <Text className="text-[11px] font-black text-foreground">OPEN DISPATCH BOARD</Text>
                    </Pressable>
                  </View>
                )}
              </>
              ...
              <>
                ...
                {availableTasks.length > 0 ? (
                  availableTasks.map((item) => (
                    <View key={item.id} className="border rounded-lg p-4 gap-3 bg-card border-border">
                      <View className="flex-row justify-between items-center">
                        <View>
                          <Text className="text-base font-black font-mono tabular-nums text-foreground">{item.id}</Text>
                          <Text className="text-[9px] font-extrabold mt-0.5 tracking-wider text-muted">
                            {item.packageCategory.toUpperCase()} • {item.weight.toFixed(1)} KG
                          </Text>
                        </View>
                      ...
                      {item.notes ? (
                        <View className="border-l-2 pl-2 py-0.5 bg-[#71717A1F] border-l-muted">
                          <Text className="text-[11px] italic text-muted" numberOfLines={1}>
                            Notes: "{item.notes}"
                          </Text>
                        </View>
                      ) : null}
                      ...
                      >
                        ...
                        <Text className="text-[11px] font-black text-foreground">ACCEPT ROUTE DISPATCH</Text>
                      </Pressable>
                    </View>
                  ))
                ) : (
